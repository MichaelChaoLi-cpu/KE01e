...
The Reviewer 4 Comment 5 missing-location sensitivity retains the ten observed emergency-water destinations and preserves the unresolved municipality counts of 18 Yatsushiro, six Uki, and two Hikawa announcements. For each of 50 placement replicates, unresolved destinations are sampled uniformly without replacement from eligible populated 125 m meshes inside their source municipality and evaluated with five prespecified seeds × 250 common Heavy-scenario closure draws per seed. This is a declared uniform-over-eligible-mesh stress test, not coordinate recovery or an assumption that the actual missing locations are uniform. The matched observed-only diagnostic gives 7,976.1 expected affected residents; the placement P05–median–P95 range is 7,052.5–7,322.2–7,936.2, equivalent to −11.6% to −0.5% relative change. Baseline-eligible population remains 1,638,212, no population becomes newly eligible, community-frequency Spearman correlation is 0.725–0.736–0.998, and Top-30 population-burden overlap is 66.7%–70.0%–96.7%. A same-seed sentinel changes only emergency-water roots and leaves shelter, fire-service, and municipal-facility loss, excess-time, and baseline arrays exactly unchanged in 9/9 checks because service classes are estimated separately. Therefore, the sensitivity bounds the aggregate emergency-water result but shows that exact local water-priority membership is less stable; it cannot identify the actual locations, urban–rural bias, availability, capacity, or operability of unresolved points. The frozen specification is `Rev/docs/analysis-spec-reviewer-4-comment-5.md`; the reproducible script is `src/analyses/revision_emergency_water_missingness_sensitivity.py`; outputs are stored under `data/exp/revision/reviewer-4-comment-5/`. The repeated-run SHA-256 values are `ac7696cd06c06431cbdf8d5e76da0e5827125edb6e106bd5f1a297755a375ae7` for the placement results, `96239d2de972e46724cecc74a008634b359832129fe83bb2433e39178841b7cf` for the summary, `381739012f9d463ddfff1374afe5a897f95256132dcfe70e6979a15988107488` for the input manifest, `6a7eaa8a30a989697704faed30895e4719bd597c50d9b205b09c3ae7e462173c` for the decision record, and `cc230550a61d1c286ea23b87c81ade4d6f6b91a333efffff2fae1224f4a8ad68` for the audit report.
...
The Reviewer 4 Comment 3 decomposition confirms that Central equality is an algebraic identity: both scores use the same consequence proxy and action assignment, and each action's median cross-setting effect-to-cost ratio equals its Central ratio. A different consequence proxy shared identically by both branches would preserve that equality. Uniform cost scaling changes affordability while preserving order; equal-action and length-only cost structures retain 70.0% and 40.0% of the Central Top 30. These membership changes do not demonstrate superiority over a comparator using the same inputs. Divergence requires breaking the ratio identity or introducing independently specified portfolio interactions, marginal benefits, constraints, or stakeholder-weighted objectives. Nonlinear or site-specific costs alone do not establish an advantage if both branches still produce the same score. These are prospective extensions requiring local evidence, not additional models tested here. The decomposition is recorded in `data/exp/revision/reviewer-4-comment-3/` and reproduced by `src/analyses/revision_intervention_identity_and_divergence_audit.py`.
